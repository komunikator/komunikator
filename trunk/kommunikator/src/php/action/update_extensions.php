...
if ($prior_values)
    ...
    $sql = "update group_priority set priority = $prior_value, group_id =(SELECT group_id FROM groups WHERE groups.group = '$group')  where extension_id = $extension_id";
     query($sql); 
     $sql ="insert into group_priority (group_id, extension_id, priority) select (SELECT group_id FROM groups WHERE groups.group = '$group'), $extension_id, $prior_value from dual where not exists (select 1 from group_priority where extension_id = $extension_id and group_id = (SELECT group_id FROM groups WHERE groups.group = '$group'))";
       ;
   }query($sql);
