...
var oSipStack = window.oSipStack || null; 
        var oSipSessionRegister, oSipSessionCall, oSipSessionTransferCall;
...
function history() {
    ...
    req.onload = function() {
        ...
        var numb = "(" + localStorage['org.doubango.identity.impi'] + ")";
        for (var i = 0; i < data.length; i++) {
            ...
            td1.setAttribute('class', 'call_logs_time_style_GLaRh');
            var td2 = document.createElement("TD");
            td2.appendChild(document.createTextNode(caller));
            var td3 = document.createElement("TD");
            td3.appendChild(document.createTextNode(called));
            ($.trim(caller) == numb) ? status = 'status2' : status = 'status1';
            var td4 = document.createElement('div');
            td4.setAttribute('class', 'statuses_hmDdi onBs_img_' + status);
            td1.setAttribute('colspan', "3", 0);
            td1.setAttribute('style', ' width: 230px;');
            row.appendChild(td2);
            row.appendChild(td4);
            row.appendChild(td3);
            ...
            row1.setAttribute('style', 'border-bottom: 1px solid #919191;');
    ...
    console.log(localStorage['service_url'] + "?action=get_call_logs" + "&session=" + localStorage['session_id'] + "&filter=%5B%7B%22type%22%3A%22date%22%2C%22comparison%22%3A%22eq%22%2C%22value\":" + JSON.stringify(today.toLocaleString()) + ",\"field%22%3A%22time%22%7D%5D&page=1&start=0&size=50&sort=time&dir=DESC");
...
function list_extensions() {
    ...
    req.onload = function() {
        ...
        for (var i = 0; i < extensions.length; i++) /*заполняем таблицу сотрудников*/
        {
            ...
            row.setAttribute('class', 'extensions_row_ulzUj');
            circle.setAttribute('class', 'circle_' + extension_status);
            td1.appendChild(circle);
            td2.appendChild(document.createTextNode(extension_number));
            td3.appendChild(document.createTextNode(name + ' ' + surname));
            phone.setAttribute('class', 'shape_wCBQb lUshc_img_phone_' + extension_status);
            td4.appendChild(phone);
            td2.setAttribute('class', 'asd');
            row.appendChild(td1);
            row.appendChild(td2);
            row.appendChild(td3);
            row.appendChild(td4);
            row.appendChild(td5);
            // row.setAttribute('ondblclick', alert(1));
            //row.setAttribute('ondblclick', 'call_ext($(\'td:nth-child(2)\', this).html(), $(\'td:nth-child(5)\', this).html());');
            //      row.setAttribute('style', 'padding-top: 5px; padding-top: 5px; padding-bottom: 5px;');
            tbody.appendChild(row);
    ...
    //  req.open("GET", "http://" + localStorage['org.doubango.identity.realm'] + "//kommunikator/data.php?action=get_list_extensions&extension='" + localStorage['org.doubango.identity.impi'] + "'&time_offset=" + time_offset + "&period=" + localStorage['refresh'] + "&" + localStorage['session_name'] + '=' + localStorage['session_id'], true);
    //req.open("GET", localStorage['org.doubango.identity.realm']+"?action=get_list_extensions&extension="+localStorage['org.doubango.identity.impi']+localStorage['org.doubango.identity.impi'] + "'&time_offset=" + time_offset + "&period=" + localStorage['refresh'] + "&" + localStorage['session_name'] + '=' + localStorage['session_id'], true);
...
;
function call_ext(e, i)
{
    alert(e + "!!!!!!!!!" + i);
    /*if (i == 'online')
     {
     $('#menu_img1').click();
     //var t = $('#txtPhoneNumber').val();
     $('#txtPhoneNumber').val(e);
     sipCall("call-audio");
     }
     else
     {
     alert('Абонент сейчас недоступен!');
     }*/
}
...
function get_session_id(cb) {
    ...
    // var t= localStorage['service_url']+"?action=auth&extension="+localStorage['extension']+"&password="+localStorage['password']; alert(t);
//localStorage['service_url'] = "http://172.17.2.147/service/data.php";
...
window.onload = function() { 
    ...
    $('.phone_tab_hgQVe li a').click(function() {
        $('.phone_tab_hgQVe li').removeClass('current');
        $(this).parent().addClass('current');
    });
    /*обрабатываем нажатие на кнопки номеронабирателя
     $('.numb_button_mpDpo').on('click', function() {
     var l = this.value;
     var t = $('#txtPhoneNumber').val(); alert(l);
     $('#txtPhoneNumber').val(t + l);
     });*/
    $('#num1_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + 1);
        sipSendDTMF('1');
    });
    $('#num2_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + 2);
        sipSendDTMF('2');
    });
    $('#num3_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + 3);
        sipSendDTMF('3');
    });
    $('#num4_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + 4);
        sipSendDTMF('4');
    });
    $('#num5_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + 5);
        sipSendDTMF('5');
    });
    $('#num6_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + 6);
        sipSendDTMF('6');
    });
    $('#num7_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + 7);
        sipSendDTMF('7');
    });
    $('#num8_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + 8);
        sipSendDTMF('8');
    });
    $('#num9_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + 9);
        sipSendDTMF('9');
    });
    $('#num0_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + 0);
        sipSendDTMF('0');
    });
    $('#numStar_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + '*');
        sipSendDTMF('*');
    });
    $('#numGrille_aOoiP').on('click', function() {
        var t = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(t + '#');
        sipSendDTMF('#');
    });
    ...
    /*	Delete */
    $('.delete_lAeQF').click(function() {
        var num = $('#txtPhoneNumber').val();
        $('#txtPhoneNumber').val(num.slice(0, -1));
    });
    /*--------------------------------------------*/
    $('#call').on('click', function() {
        sipCall("call-audio");
    });
    $('.phone_tab_hgQVe li a').on('click', function() {
        var id = $(this).attr('id');
        default_img_page();
       // $('#' + id).addClass("lnybt_active_" + id);
        $('#page_' + id).show();
    });
    $('#menu_img3').on('click', function() {
        list_extensions();
    });
    $('#menu_img2').on('click', function() {
        history();
    });
    $('#menu_img1').click();
    $('.arrow_KeVNB').on('click', function() {
        history();
    });
    ...
    $('#btnRegister').on('click', function() {
        settingsSave();
        document.getElementById("txtPublicIdentity").value = 'sip:' + txtPrivateIdentity.value + '@' + txtRealm.value;
        sipRegister();
        save_options();
    });
    ...
    document.getElementsByClassName('extensions_row_ulzUj').onclick = function() {

        alert('changed');
    };
    /*$('.extensions_row_ulzUj').bind('click', function(){
...
function default_img_page() {
    for (var i = 1; i <= 4; i++) {
       // $("#menu_img" + i).removeClass("lnybt_active_menu_img" + i);
       // $("#menu_img" + i).addClass("ojTHw_menu_img" + i);
        $('#page_menu_img' + i).hide();
    }
}
function postInit() {
...
function saveCredentials() {
    if (window.localStorage) {
        ...
        window.localStorage.setItem('org.doubango.identity.impu', 'sip:' + txtPrivateIdentity.value + '@' + txtRealm.value);
    }
}
;
        ...
        default:
            {
                ...
                // btnCall.setAttribute("class", "btn btnCall");
